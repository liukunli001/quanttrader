...
from quantrader import TimeFrame, date2num, num2date
from quantrader.feed import DataBase
from quantrader.metabase import MetaParams
from quantrader.utils.py3 import (integer_types, queue, string_types,
                                  with_metaclass)

from quantrader.stores import vcstore
...
class VCData(with_metaclass(MetaVCData, DataBase)):
    ...
    Params:
      ...
      - ``usetimezones`` (default: ``True``)
        ...
        allows for datetime to be converted to market time (*quantrader* choice
        for representation)
    ...
    def islive(self):
        '''Returns ``True`` to notify ``Engine`` that preloading and runonce
    ...
    def setenvironment(self, env):
        '''Receives an environment (engine) and passes it over to the store it
        belongs to'''
