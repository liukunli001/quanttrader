...
from quantrader import (date2num, num2date, time2num, TimeFrame, dataseries,
                        metabase)

from quantrader.utils.py3 import with_metaclass, zip, range, string_types
from quantrader.utils import tzparse
...
class MetaAbstractDataBase(dataseries.OHLCDateTime.__class__):
    ...
    def dopreinit(cls, _obj, *args, **kwargs):
        ...
        _obj.notifs = collections.deque()  # store notifications for engine
...
class AbstractDataBase(with_metaclass(MetaAbstractDataBase,
    ...
    def islive(self):
        '''If this returns True, ``Engine`` will deactivate ``preload`` and
