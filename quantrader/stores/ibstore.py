...
from quantrader import TimeFrame, Position
from quantrader.metabase import MetaParams
from quantrader.utils.py3 import bytes, bstr, queue, with_metaclass, long
from quantrader.utils import AutoDict, UTC
...
class IBStore(with_metaclass(MetaSingleton, object)):
    ...
    Params:
      ...
      - ``notifyall`` (default: ``False``)
        ...
        ``notify_store`` methods of ``Engine`` and ``Strategy``.
      ...
      - ``timeoffset`` (default: ``True``)
        ...
        The time offset will propagate to other parts of the ``quantrader``
    ...
    def __init__(self):
        ...
        self.ccount = 0  # requests to start (from engine or datas)
        ...
        self.notifs = queue.Queue()  # store notifications for engine
    ...
    def error(self, msg):
        ...
        # All errors are logged to the environment (engine), because many
