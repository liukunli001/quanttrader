...
class MetaStrategy(StrategyBase.__class__):
    ...
    def donew(cls, *args, **kwargs):
        ...
        _obj.env = _obj.engine = engine = findowner(_obj, bt.Engine)
    ...
    def buy(self, data=None,
        ...
        '''Create a buy (long) order and send it to the broker
          ...
          - ``valid`` (default: ``None``)
            ...
            Possible values:
              ...
              - ``numeric value``: This is assumed to be a value corresponding
                ...
                ``quantrader``) and will used to generate an order valid until
                that time (*good till date*)

          - ``tradeid`` (default: ``0``)

            This is an internal value applied by ``quantrader`` to keep track
          ...
          - ``**kwargs``: additional broker implementations may support extra
            parameters. ``quantrader`` will pass the *kwargs* down to the
