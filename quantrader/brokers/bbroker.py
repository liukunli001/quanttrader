...
from quantrader.comminfo import CommInfoBase
from quantrader.order import Order, BuyOrder, SellOrder
from quantrader.position import Position
from quantrader.utils.py3 import string_types, integer_types
...
class BackBroker(bt.BrokerBase):
    '''Broker Simulator
      ...
      for each iteration of ``engine`` and keeping the current position on
      different datas.
      ...
      *cash* is adjusted on each iteration for instruments like ``futures`` for
      ...
      Because the broker is instantiated by ``Engine`` and there should be
      ...
      by the user for the instance.  To change this there are two options:
        ...
        1. Manually create an instance of this class with the desired params
           and use ``engine.broker = instance`` to set the instance as the
           broker for the ``run`` execution

        2. Use the ``set_xxx`` to set the value using
           ``engine.broker.set_xxx`` where ```xxx`` stands for the name of the
           parameter to set

        .. note::

           ``engine.broker`` is a *property* supported by the ``getbroker``
           and ``setbroker`` methods of ``Engine``
    ...
    def _process_fund_history(self):
        ...
        # if possible
        # st0 = self.engine.runningstrats[0]
        # if dt <= st0.datetime.datetime():
        if dt <= self.engine._dtmaster:
            self._fhistlast = f[1:]
    ...
    def _process_order_history(self):
        for uhist in self._userhist:
            ...
            while uhorder is not None:
                ...
                if dataidx is None:
                    d = self.engine.datas[0]
                elif isinstance(dataidx, integer_types):
                    d = self.engine.datas[dataidx]
                else:  # assume string
                    d = self.engine.datasbyname[dataidx]
                ...
                size = uhorder[1]
                price = uhorder[2]
                owner = self.engine.runningstrats[0]
                if size > 0:
