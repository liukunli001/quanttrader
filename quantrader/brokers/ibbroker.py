...
from quantrader.feed import DataBase
from quantrader import (TimeFrame, num2date, date2num, BrokerBase,
                        Order, OrderBase, OrderData)
from quantrader.utils.py3 import bytes, bstr, with_metaclass, queue, MAXFLOAT
from quantrader.metabase import MetaParams
from quantrader.comminfo import CommInfoBase
from quantrader.position import Position
from quantrader.stores import ibstore
from quantrader.utils import AutoDict, AutoOrderedDict
from quantrader.comminfo import CommInfoBase
...
class IBOrder(OrderBase, ib.ext.Order.Order):
    ...
    ib.ext.Order.Order object, which could be used as follows::
      ...
      ``quantrader`` are not enough, in the case of for example
      *Interactive Brokers* the following could be passed as *kwargs*::

        orderType='LIT', lmtPrice=10.0, auxPrice=9.8

      This would override the settings created by ``quantrader`` and
      generate a ``LIMIT IF TOUCHED`` order with a *touched* price of 9.8
      and a *limit* price of 10.0.

    This would be done almost always from the ``Buy`` and ``Sell`` methods of
    the ``Strategy`` subclass being used in ``Engine``
    '''
    ...
    # Map quantrader order types to the ib specifics
    _IBOrdTypes = {
...
class IBBroker(with_metaclass(MetaIBBroker, BrokerBase)):
    ...
    Notes:
      ...
      - Position
        ...
        calculated in the ``Strategy`` in engine will not reflect the reality.
