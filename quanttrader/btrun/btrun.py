...
try:
    DATAFORMATS['vcdata'] = trader.feeds.VCData
except AttributeError:
    pass  # no comtypes available

try:
    DATAFORMATS['ibdata'] = trader.feeds.IBData,
except AttributeError:
    pass  # no ibpy available

try:
    DATAFORMATS['oandadata'] = trader.feeds.OandaData,
except AttributeError:
...
def btrun(pargs=''):
    ...
    for data in getdatas(args):
        if args.resample is not None:
            engine.resampledata(data, timeframe=tf, compression=cp)
        elif args.replay is not None:
            engine.replaydata(data, timeframe=tf, compression=cp)
        else:
            engine.adddata(data)

    # get and add signals
    signals = getobjects(args.signals, trader.Indicator, trader.signals, issignal=True)
    for sig, kwargs, sigtype in signals:
        stype = getattr(trader.signal, 'SIGNAL_' + sigtype.upper())
        engine.add_signal(stype, sig, **kwargs)

    # get and add strategies
    strategies = getobjects(args.strategies, trader.Strategy, trader.strategies)
    for strat, kwargs in strategies:
        engine.addstrategy(strat, **kwargs)

    inds = getobjects(args.indicators, trader.Indicator, trader.indicators)
    for ind, kwargs in inds:
        engine.addindicator(ind, **kwargs)

    obs = getobjects(args.observers, trader.Observer, trader.observers)
    for ob, kwargs in obs:
        engine.addobserver(ob, **kwargs)

    ans = getobjects(args.analyzers, trader.Analyzer, trader.analyzers)
    for an, kwargs in ans:
        engine.addanalyzer(an, **kwargs)

    setbroker(args, engine)

    for wrkwargs_str in args.writers or []:
        wrkwargs = eval('dict(' + wrkwargs_str + ')')
        engine.addwriter(trader.WriterFile, **wrkwargs)

    ans = getfunctions(args.hooks, trader.Engine)
    ...
    if args.plot:
        ...
        # engine.plot(numfigs=args.plotfigs, style=args.plotstyle)
        engine.plot(**pkwargs)


def setbroker(args, engine):
    broker = engine.getbroker()
...
def parse_args(pargs=''):
    ...
    group = parser.add_argument_group(title='Engine options')
    group.add_argument(
        '--engine', '-cer',
        metavar='kwargs',
        ...
        help=('The argument can be specified with the following form:\n'
              ...
              '\n'
              'The passed kwargs will be passed directly to the engine\n'
              'instance created for the execution\n'
              '\n'
              'The available kwargs to engine are:\n'
    ...
    group.add_argument(
        ...
        help=('This option can be specified multiple times.\n'
              ...
              'as the built-in engine method. Example:\n'
              '\n'
    ...
    group.add_argument(
        ...
        help=('This option can be specified multiple times.\n'
              ...
              'fromt the trader.signal module\n'
              '\n'
              'If module is omitted then class name will be sought in\n'
