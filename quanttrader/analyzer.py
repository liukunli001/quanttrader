...
class MetaAnalyzer(trader.MetaParams):
    def donew(cls, *args, **kwargs):
        ...
        _obj.strategy = strategy = trader.metabase.findowner(_obj, trader.Strategy)
        _obj._parent = trader.metabase.findowner(_obj, Analyzer)

        # Register with a master observer if created inside one
        masterobs = trader.metabase.findowner(_obj, trader.Observer)
...
class Analyzer(with_metaclass(MetaAnalyzer, object)):
    ...
    def print(self, *args, **kwargs):
        ...
        output
        '''
        writer = trader.WriterFile(*args, **kwargs)
        writer.start()
        pdct = dict()
