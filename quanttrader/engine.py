...
# Defined here to make it pickable. Ideally it could be defined inside Engine
...
class Engine(with_metaclass(MetaParams, object)):
    '''Params:

      - ``preload`` (default: ``True``)

        Whether to preload the different ``data feeds`` passed to engine for
        the Strategies
      ...
      - ``cheat_on_open`` (default: ``False``)
        ...
        call ``engine.broker.set_coo(True)`` or instantite a broker with
        ``BackBroker(coo=True)`` (where *coo* stands for cheat-on-open) or set
        the ``broker_coo`` parameter to ``True``. Engine will do it
            ...
            where:
              ...
              - ``data`` if present can take any of the following values
                ...
                - *string* - a data with that name, assigned for example with
                  ``engine.addata(data, name=value)``, will be the target
    ...
    def _add_timer(self, owner, when,
        ...
        can be called by engine instances or other objects which can access
        engine'''
    ...
    def notify_store(self, msg, *args, **kwargs):
        '''Receive store notifications in engine

        This method can be overridden in ``Engine`` subclasses
    ...
    def notify_data(self, data, status, *args, **kwargs):
        '''Receive data notifications in engine
    ...
    def chaindata(self, *args, **kwargs):
        ...
        d = trader.feeds.Chainer(dataname=dname, *args)
    ...
    def rolloverdata(self, *args, **kwargs):
        ...
        if dname is None:
            dname = args[0]._dataname
        d = trader.feeds.RollOver(dataname=dname, *args, **kwargs)
        self.adddata(d, name=dname)
    ...
    def optstrategy(self, strategy, *args, **kwargs):
        ...
        purposes the call to ``optstrategy`` looks like:
          ...
          - engine.optstrategy(MyStrategy, period=(15, 25))

        This will execute an optimization for values 15 and 25. Whereas

          - engine.optstrategy(MyStrategy, period=range(15, 25))
        ...
        like:
          ...
          - engine.optstrategy(MyStrategy, period=(15,))

        Notice that ``period`` is still passed as an iterable ... of just 1
        element

        ``quanttrader`` will anyhow try to identify situations like:
    ...
    def plot(self, plotter=None, numfigs=1, iplot=True, start=None, end=None,
             ...
             **kwargs):
        '''
        Plots the strategies inside engine
    ...
    def __call__(self, iterstrat):
        '''
        Used during optimization to pass the engine over the multiprocesing
    ...
    def run(self, **kwargs):
        ...
        will affect the value of the standard parameters ``Engine`` was
        instantiated with.

        If ``engine`` has not datas the method will immediately bail out.
    ...
    def runstrategies(self, iterstrat, predata=False):
        ...
        for stratcls, sargs, skwargs in iterstrat:
            ...
            except trader.errors.StrategySkipError:
    ...
    def stop_writers(self, runstrats):
        ...
        engineinfo['Strategies'] = stratinfos

        for writer in self.runwriters:
            writer.writedict(dict(Engine=engineinfo))
            writer.stop()
