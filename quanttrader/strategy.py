...
class MetaStrategy(StrategyBase.__class__):
    ...
    def donew(cls, *args, **kwargs):
        ...
        _obj.env = _obj.engine = engine = findowner(_obj, trader.Engine)
        _obj._id = engine._next_stid()
    ...
    def dopreinit(cls, _obj, *args, **kwargs):
        ...
        _obj._sizer = trader.sizers.FixedSize()
        _obj._orders = list()
...
class Strategy(with_metaclass(MetaStrategy, StrategyBase)):
    ...
    def _addwriter(self, writer):
        ...
        because the writer works at engine level and is only passed to the
        ...
        **Note**: can be called during ``__init__`` or ``start``
        ...
        Arguments:
          ...
          - ``when``: can be
            ...
            - ``trader.timer.SESSION_START`` to reference a session start
            - ``trader.timer.SESSION_END`` to reference a session end
    ...
    def getdatabyname(self, name):
        '''
        Returns a given data by name using the environment (engine)
        '''
    ...
    def buy(self, data=None,
        ...
        '''Create a buy (long) order and send it to the broker
          ...
          - ``valid`` (default: ``None``)
            ...
            Possible values:
              ...
              - ``numeric value``: This is assumed to be a value corresponding
                ...
                ``quanttrader``) and will used to generate an order valid until
                that time (*good till date*)

          - ``tradeid`` (default: ``0``)

            This is an internal value applied by ``quanttrader`` to keep track
          ...
          - ``**kwargs``: additional broker implementations may support extra
            parameters. ``quanttrader`` will pass the *kwargs* down to the
            created order objects

            Example: if the 4 order execution types directly supported by
            ``quanttrader`` are not enough, in the case of for example
            *Interactive Brokers* the following could be passed as *kwargs*::

              orderType='LIT', lmtPrice=10.0, auxPrice=9.8

            This would override the settings created by ``quanttrader`` and
    ...
    def buy_bracket(self, data=None, size=None, price=None, plimit=None,
                    exectype=trader.Order.Limit, valid=None, tradeid=0,
                    trailamount=None, trailpercent=None, oargs={},
                    stopprice=None, stopexec=trader.Order.Stop, stopargs={},
                    limitprice=None, limitexec=trader.Order.Limit, limitargs={},
                    **kwargs):
        '''
        ...
        See below for the different parameters
          ...
          - ``exectype`` (default: ``trader.Order.Limit``)
          ...
          - ``stopexec`` (default: ``trader.Order.Stop``)
          ...
          - ``stopexec`` (default: ``trader.Order.Limit``)
    ...
    def _addsizer(self, sizer, *args, **kwargs):
        if sizer is None:
            self.setsizer(trader.sizers.FixedSize())
        else:
...
class MetaSigStrategy(Strategy.__class__):
    ...
    def dopreinit(cls, _obj, *args, **kwargs):
        ...
        elif isinstance(_data, trader.LineRoot):
            _obj._dtarget = _data
        else:
            _obj._dtarget = _obj.data0
    ...
    def dopostinit(cls, _obj, *args, **kwargs):
        ...
        _obj._longshort = bool(_obj._signals[trader.SIGNAL_LONGSHORT])

        _obj._long = bool(_obj._signals[trader.SIGNAL_LONG])
        _obj._short = bool(_obj._signals[trader.SIGNAL_SHORT])

        _obj._longexit = bool(_obj._signals[trader.SIGNAL_LONGEXIT])
        _obj._shortexit = bool(_obj._signals[trader.SIGNAL_SHORTEXIT])
    ...
    **Order Issuing**
    ...
    Params:
      ...
      - ``signals`` (default: ``[]``): a list/tuple of lists/tuples that allows
        ...
        This parameter is expected to be managed through ``engine.add_signal``
      ...
      - ``_data`` (default: ``None``): if multiple datas are present in the
        ...
        - An ``str``: name given to the data when creating it (parameter
          ``name``) or when adding it engine with ``engine.adddata(...,
          name=)``
    ...
    def _next_signal(self):
        ...
        ls_long = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_LONGSHORT] or nosig)
        ls_short = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_LONGSHORT] or nosig)

        l_enter0 = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_LONG] or nosig)
        l_enter1 = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_LONG_INV] or nosig)
        l_enter2 = all(x[0] for x in sigs[trader.SIGNAL_LONG_ANY] or nosig)
        l_enter = l_enter0 or l_enter1 or l_enter2

        s_enter0 = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_SHORT] or nosig)
        s_enter1 = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_SHORT_INV] or nosig)
        s_enter2 = all(x[0] for x in sigs[trader.SIGNAL_SHORT_ANY] or nosig)
        s_enter = s_enter0 or s_enter1 or s_enter2

        l_ex0 = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_LONGEXIT] or nosig)
        l_ex1 = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_LONGEXIT_INV] or nosig)
        l_ex2 = all(x[0] for x in sigs[trader.SIGNAL_LONGEXIT_ANY] or nosig)
        l_exit = l_ex0 or l_ex1 or l_ex2

        s_ex0 = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_SHORTEXIT] or nosig)
        s_ex1 = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_SHORTEXIT_INV] or nosig)
        s_ex2 = all(x[0] for x in sigs[trader.SIGNAL_SHORTEXIT_ANY] or nosig)
        ...
        l_leav0 = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_LONG] or nosig)
        l_leav1 = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_LONG_INV] or nosig)
        l_leav2 = all(x[0] for x in sigs[trader.SIGNAL_LONG_ANY] or nosig)
        l_leave = l_leav0 or l_leav1 or l_leav2

        s_leav0 = all(x[0] > 0.0 for x in sigs[trader.SIGNAL_SHORT] or nosig)
        s_leav1 = all(x[0] < 0.0 for x in sigs[trader.SIGNAL_SHORT_INV] or nosig)
        s_leav2 = all(x[0] for x in sigs[trader.SIGNAL_SHORT_ANY] or nosig)
